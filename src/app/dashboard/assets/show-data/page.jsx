        ...
        <tbody>
          {assets.length > 0 ? (
            assets.map((asset, index) => {
              const status = determineAssetStatus(asset.usefulLife, asset.purchaseDate);

              return (
                <tr key={index}>
                  <td className="border px-4 py-2">{asset.name}</td>
                  <td className="border px-4 py-2">{asset.assetType}</td>
                  <td className="border px-4 py-2">${parseFloat(asset.purchaseCost).toFixed(2)}</td>
                  <td className="border px-4 py-2">{asset.usefulLife} years</td>
                  <td className="border px-4 py-2">{asset.depreciationRate}%</td>
                  <td className="border px-4 py-2">${parseFloat(asset.currentValue).toFixed(2)}</td>
                  <td className="border px-4 py-2">{asset.purchaseDate}</td>
                  <td className={`border px-4 py-2 ${status === 'Expired' ? 'text-red-600' : 'text-green-600'}`}>
                    {status} 
                  </td>
            ...
            <tr>
              <td colSpan="9" className="text-center p-4">No asset data available</td>
            </tr>
          )}
        </tbody>
      </table>
